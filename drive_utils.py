...
if user_input or uploaded_image:
    ...
    # 💡【UI改善1】API処理の前に、ユーザーの発言を画面に即座に表示！
    ...
    # 💡【UI改善2】ユーザーの発言のすぐ下に「考え中」を表示しながらGeminiを呼び出す
    with st.chat_message("model", avatar="🤖"):
        with st.spinner("J.A.R.V.I.S. が思考中..."):
            try:
                ...
                # 過去の全会話履歴を文脈（コンテキスト）としてGeminiに教える
                ...
                # 次回の画面再描画（rerun）に備えて、直近1往復のみをdisplay_historyにセット
                st.session_state.display_history = [user_msg, ai_msg]
                
                # 食事ログと会話記憶を Google Drive へ追記保存
                if uploaded_image:
